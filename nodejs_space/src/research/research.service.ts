...
export class ResearchService {
  ...
  async backtestSignals(params: BacktestParams) {
    ...
    if (params.fromDate || params.toDate) {
      where.date = {};
      if (params.fromDate) {
        (where.date as Record<string, Date>).gte = normalizeDateOnly(
          params.fromDate,
        );
      }
      if (params.toDate) {
        (where.date as Record<string, Date>).lte = normalizeDateOnly(
          params.toDate,
        );
      }
    }
    ...
    const earliestSignalDate = signals.reduce(
      (earliest, signal) =>
        signal.date.getTime() < earliest.getTime() ? signal.date : earliest,
      signals[0].date,
    );
    ...
    const trades = signals.map((signal) =>
      this.evaluateTrade(
        signal,
        barsBySymbol.get(signal.symbol) ?? [],
        params.horizonDays,
      ),
    );
    const completedTrades = trades.filter(
      (trade) => trade.status === 'completed' && trade.return_pct !== null,
    );
    const openTrades = trades.filter(
      (trade) => trade.status === 'open_no_exit',
    );
    const missingEntryTrades = trades.filter(
      (trade) => trade.status === 'missing_entry_bar',
    );
    ...
    return {
      ...
      summary: {
        ...
        average_return_pct: average(
          completedTrades.map((trade) => trade.return_pct as number),
        ),
        ...
        win_rate_pct: completedTrades.length
          ? roundPct(
              (completedTrades.filter(
                (trade) => (trade.return_pct as number) > 0,
              ).length /
      ...
      leaders: {
        best_completed_trade: completedTrades.length
          ? sortDescending(
              completedTrades,
              (trade) => trade.return_pct as number,
            )[0]
          : null,
  ...
  private evaluateTrade(
    ...
    const entryIndex = bars.findIndex(
      (bar) => normalizeDateOnly(bar.date).getTime() > signalTimestamp,
    );
    ...
    if (entryIndex === -1) {
      return {
        ...
        exit_close: null,
        return_pct: null,
        latest_available_date: latestBar
          ? formatDateOnly(latestBar.date)
          : null,
    ...
    const markToMarketReturn = calculateReturnPct(
      entryBar.open,
      latestAfterEntry?.close ?? null,
    );
    ...
    return {
      ...
      return_pct: exitBar
        ? calculateReturnPct(entryBar.open, exitBar.close)
        : null,
      latest_available_date: latestAfterEntry
        ? formatDateOnly(latestAfterEntry.date)
        : null,
      latest_available_close: latestAfterEntry
        ? roundValue(latestAfterEntry.close)
        : null,
