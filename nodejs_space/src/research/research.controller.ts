...
export class ResearchController {
  ...
  @ApiOperation({
    ...
    description:
      'Uses next-trading-day open as entry and horizon-day close as exit. Only persisted signal and market_bar data are used.',
  })
  @ApiQuery({
    name: 'run_id',
    required: false,
    type: String,
    description: 'Limit backtest to a single analysis run',
  })
  @ApiQuery({
    name: 'symbol',
    required: false,
    type: String,
    description: 'Limit backtest to one symbol',
  })
  @ApiQuery({
    name: 'selected_only',
    required: false,
    type: Boolean,
    description: 'Only evaluate selected signals (default true)',
  })
  @ApiQuery({
    name: 'from_date',
    required: false,
    type: String,
    description: 'Signal start date (YYYY-MM-DD)',
  })
  @ApiQuery({
    name: 'to_date',
    required: false,
    type: String,
    description: 'Signal end date (YYYY-MM-DD)',
  })
  @ApiQuery({
    name: 'horizon_days',
    required: false,
    type: Number,
    description: 'Holding horizon in trading days (default 5)',
  })
  @ApiQuery({
    name: 'limit',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description: 'Backtest summary and trade-level outcomes',
  })
  async backtestSignals(
    ...
    const selectedOnlyValue =
      parseBooleanQuery(selectedOnly, 'selected_only') ?? true;
    const parsedFromDate = parseDateQuery(fromDate, 'from_date');
    const parsedToDate = parseDateQuery(toDate, 'to_date');
    const horizon = parseIntegerQuery(horizonDays, 'horizon_days', 5, {
      min: 1,
      max: 30,
    });
