...
export class PositionService {
  ...
  async snapshotAllOpenPositions(): Promise<{
    ...
    for (const trade of openTrades) {
      try {
        ...
        const daysToExpiry = Math.max(
          0,
          Math.ceil(
            (earliestExpiry.getTime() - today.getTime()) /
              (1000 * 60 * 60 * 24),
          ),
        );

        // Estimate mark value and Greeks from entry data
        const markValue = this.estimateMarkValue(
          trade as unknown as TradeForSnapshot,
          price,
        );
        const pnlUnrealized =
        ...
        await this.prisma.position_snapshot.create({
          data: {
            ...
            delta: this.estimatePositionDelta(
              trade as unknown as TradeForSnapshot,
              price,
              daysToExpiry,
            ),
            theta: this.estimatePositionTheta(
              trade as unknown as TradeForSnapshot,
              daysToExpiry,
            ),
            vega: null,
        ...
        errors.push(
          `Error snapshotting trade ${trade.id} (${trade.symbol}): ${message}`,
        );
    ...
    this.logger.log(
      `Snapshots created: ${snapshotsCreated}/${openTrades.length}`,
    );
  ...
  async getPositionTimeline(tradeId: number): Promise<
    Array<{
      snapshot_date: Date;
  ...
  private estimateMarkValue(
    trade: TradeForSnapshot,
    ...
    for (const leg of legs) {
      ...
      const estimatedPremium =
        intrinsicValue + (leg.entry_price ?? 0) * Math.sqrt(timeValueRatio);
      markEstimate +=
        leg.side === 'sell' ? estimatedPremium : -estimatedPremium;
    }

    return Math.max(0, markEstimate);
  }

  private estimatePositionDelta(
    trade: TradeForSnapshot,
    underlyingPrice: number,
    dte: number,
  ): number {
    ...
    for (const leg of trade.legs) {
      const delta =
        leg.delta_at_entry ?? (leg.option_type === 'put' ? -0.16 : 0.16);
      netDelta +=
        leg.side === 'sell' ? -delta * leg.quantity : delta * leg.quantity;
    }
  ...
  private estimatePositionTheta(trade: TradeForSnapshot, dte: number): number {
    ...
    return (
      Math.round(
        (entryCredit / Math.max(dte, 1)) * 100 * trade.contracts * 100,
      ) / 100
    );
  }
}
