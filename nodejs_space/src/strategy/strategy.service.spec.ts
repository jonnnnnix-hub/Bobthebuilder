...
describe('StrategyService', () => {
  ...
  describe('suggestStrategy', () => {
    ...
    it('should suggest short_put when VRP high and IV moderate', () => {
      ...
      expect(result.target_delta_short).toBe(0.20);
    });
  ...
  describe('calculateLegs', () => {
    it('should return 1 leg for short_put', () => {
      const legs = service.calculateLegs('short_put', 450, 0.20, 0.20);
    ...
    it('should throw for non-positive underlying price', () => {
      expect(() => service.calculateLegs('short_put', 0, 0.20, 0.20)).toThrow('underlyingPrice must be positive');
      expect(() => service.calculateLegs('short_put', -100, 0.20, 0.20)).toThrow('underlyingPrice must be positive');
    });

    it('should throw for non-positive IV', () => {
      expect(() => service.calculateLegs('short_put', 450, 0, 0.20)).toThrow('atmIv must be positive');
    });
  });

  describe('calculateMaxLoss', () => {
    it('should calculate max loss for short_put', () => {
      const legs = [{ option_type: 'put' as const, strike: 430, side: 'sell' as const, delta: -0.2 }];
      const maxLoss = service.calculateMaxLoss('short_put', legs, 2.50, 1);
      // (430 - 2.50) * 100 = 42750
    ...
    it('should calculate max loss for iron_condor', () => {
      const legs = [
        { option_type: 'put' as const, strike: 420, side: 'buy' as const, delta: -0.05 },
        { option_type: 'put' as const, strike: 430, side: 'sell' as const, delta: -0.16 },
        { option_type: 'call' as const, strike: 470, side: 'sell' as const, delta: 0.16 },
        { option_type: 'call' as const, strike: 480, side: 'buy' as const, delta: 0.05 },
      ];
      const maxLoss = service.calculateMaxLoss('iron_condor', legs, 3.00, 1);
    ...
    it('should scale max loss by contracts', () => {
      ...
      const maxLoss = service.calculateMaxLoss('iron_condor', legs, 3.00, 5);
      // (10 - 3) * 500 = 3500
  ...
  describe('calculateBreakevens', () => {
    it('should calculate breakeven for short_put', () => {
      const legs = [{ option_type: 'put' as const, strike: 430, side: 'sell' as const, delta: -0.2 }];
      const result = service.calculateBreakevens('short_put', legs, 2.50);
    ...
    it('should calculate breakevens for short_strangle', () => {
      ...
      const result = service.calculateBreakevens('short_strangle', legs, 5.00);
    ...
    it('should calculate breakevens for iron_condor', () => {
      ...
      const result = service.calculateBreakevens('iron_condor', legs, 3.00);
